...
class MagentoWebsite(models.Model):
    ...
    def get_magento_invoice_records(self, state):
        ...
        magento_invoice_data.update({'order_count': len(magento_invoice_ids), 'order_action': action})
    ...
    def get_magento_picking_records(self, state):
        ...
        magento_picking_data.update({'order_count': len(magento_picking_ids), 'order_action': action})
    ...
    def get_compare_data(self, record):
        ...
        if website_current_total > 0.0:
            if website_current_total >= website_previous_total:
                data_type = 'positive'
                total_percentage = (website_current_total - website_previous_total) * 100 / website_current_total
            if website_previous_total > website_current_total:
                data_type = 'negative'
                total_percentage = (website_previous_total - website_current_total) * 100 / website_current_total
