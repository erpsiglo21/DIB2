...
class MagentoInstance(models.Model):
    ...
    def check_dashboard_view(self):
        ...
        if view_type == 'instance_level':
            view = \
            self.env.ref('odoo_magento2_ept.action_magento_dashboard_instance').sudo().read()[0]
        else:
            view = self.env.ref('odoo_magento2_ept.action_magento_dashboard_website').sudo().read()[
                0]
        action = self.prepare_action(view, [])
        return action

    def _compute_get_scheduler_list(self):
        seller_cron = self.env[IR_CRON].sudo().search([('magento_instance_id', '=', self.id)])
        for record in self:
    ...
    def _scheduler_update_product_stock_qty(self, args=None):
        ...
        if magento_instance_id:
            ...
            if instance.magento_version in ['2.1',
                ...
                magento_product_product.export_product_stock_to_multiple_locations(instance,
                                                                                   inventory_locations)
    ...
    def get_draft_refund(self):
        ...
        state = "('{}')".format(context.get('state'))
    ...
    def _prepare_website_refund_query(website_id, instance, state):
        ...
        SELECT r.id 
            ...
            WHERE r.move_type = 'out_refund'
                ...
                AND mw.id = {}
                AND r.state in {}
        """.format(website_id, instance.id, state)
        return query
    ...
    def _prepare_refund_query(instance, state=('draft', 'posted', 'cancel')):
        ...
        SELECT DISTINCT(id) 
            ...
            WHERE move_type = 'out_refund' 
                AND magento_instance_id = {}
                AND state in {}
        """.format(instance.id, state)
        return query
